...
fn test_simple_sequence() {
    ...
    let defs_out0_kinds: HashSet<_> = flow0.defs_out.iter().map(|def| def.kind.clone()).collect();
    assert!(
    ...
    // Reaching Defs
    let defs_in12_kinds: HashSet<_> = flow12.defs_in.iter().map(|def| def.kind.clone()).collect();
    assert!(
...
fn test_if_else() {
    ...
    let defs_in20_kinds: HashSet<_> = flow20.defs_in.iter().map(|def| def.kind.clone()).collect();
    assert!(
    ...
    let defs_in16_kinds: HashSet<_> = flow16.defs_in.iter().map(|def| def.kind.clone()).collect();
    ...
    let defs_out20_kinds: HashSet<_> = flow20.defs_out.iter().map(|def| def.kind.clone()).collect();
    assert!(
    ...
    let defs_in22_kinds: HashSet<_> = flow22.defs_in.iter().map(|def| def.kind.clone()).collect();
    assert!(
...
fn test_function_call_return_values() {
    ...
    let defs_in21_kinds: HashSet<_> = flow21.defs_in.iter().map(|def| def.kind.clone()).collect();
...
fn test_multiple_function_calls() {
    ...
    let check_returns_in = |model: &Model<DataFlowComplete>,
        ...
        flow.function_returns_in.iter().any(|fc| {
            fc.calling_block == calling_block && fc.function_addr == Expression::Constant(func_addr)
        })
    };
