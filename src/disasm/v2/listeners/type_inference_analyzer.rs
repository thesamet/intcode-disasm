...
use crate::disasm::v2::{
    ...
    events::{Event, SsaConversionComplete, TypeInferenceComplete},
    model::ProgramModel,
    ssa_form::SsaProgram,
    type_inference::{TypeInference, Type, TypeVarId},
};
            ...
            Event::SsaConversionComplete(_) => {
                ...
                if let Some(ssa_converter) = ssa_converter {
                    if let Some(ssa_program) = ssa_converter.get_ssa_program() {
                        ...
                        self.type_inference.generate_constraints_for_program(ssa_program);
                        ...
                        match self.type_inference.unify() {
                            Ok(substitution) => {
                                ...
                                collector.publish(TypeInferenceComplete {
                                    completed: true,
                                });
                            }
                            Err(error) => {
