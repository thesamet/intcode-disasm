...
pub fn traverse_expression<V: ExpressionVisitor>(expr: &HlrExpression, visitor: &mut V) -> HlrExpression {
    ...
    let result = match expr {
        ...
        HlrExpression::BinaryOp { op, left, right, result_type } => {
        ...
        HlrExpression::FunctionCall(func, args) => {
            ...
            let changed = **func != new_func || 
                args.iter().zip(new_args.iter()).any(|(a, b)| a != b);
        ...
        HlrExpression::Tuple(elements) => {
            ...
            let changed = elements.iter().zip(new_elements.iter())
                .any(|(a, b)| a != b);
...
pub fn traverse_statement<V: StatementVisitor>(stmt: &HlrStatement, visitor: &mut V) -> Option<HlrStatement> {
    match stmt {
...
pub fn traverse_statements<V: StatementVisitor>(stmts: &[HlrStatement], visitor: &mut V) -> Vec<HlrStatement> {
...
pub fn map_statements<F, E>(stmts: &[HlrStatement], expr_visitor: &mut E, f: &mut F) -> Vec<HlrStatement> 
where 
